...
**Шаг 1:** Написать функцию-эффект. Сигнатура НЕ принимает номер тика — фазы и позиции продвигаются общими инкрементными счётчиками библиотеки (`fx_phase`, `fx_pos`, `fx_cnt`) и хелпером `fx_adv()` без деления:

```c
/**
 * Мой эффект: бегущий огонь 0→1→2→3→0, 1 позиция каждые 5 тиков.
 */
static void eff_my_custom(bool st[LED_COUNT]) {
    uint8_t i;
...
| Компонент | Размер |
|-----------|--------|
| `leds[4]` (конечные автоматы) | 4 × 20 = 80 байт |
