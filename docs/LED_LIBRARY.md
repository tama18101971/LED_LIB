...
Скопируйте `led.h` и `led.c` в свой проект.
...
**Шаг 1:** Написать функцию-эффект:

```c
/**
 * Мой эффект: все диоды включаются по очереди с нарастающей скоростью.
 */
static void eff_my_custom(uint32_t tick, bool st[LED_COUNT]) {
    // Ускорение: каждые 100 тиков (1.67 сек) интервал уменьшается
    uint32_t speed = 20 - (tick / 100);
    if (speed < 2) speed = 2;  // Минимальный интервал

    uint8_t pos = (tick / speed) % LED_COUNT;
    uint8_t i;
...
1. **Скопировать** `led.h` и `led.c` в проект
...
|-----------|------------------------|
| Библиотека (ядро + эффекты) | ~2.5 КБ |
